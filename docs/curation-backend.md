...
> 每日 AI 策展已经改为**只追加建议、人工终审**。它不会自动创建 collect job，也不会发布或永久拒绝候选。运行、失败与回滚规范见 [`daily-curation-operations.md`](./daily-curation-operations.md)。下方“一键收录”仅指人工在后台确认后进入既有收录流程。
